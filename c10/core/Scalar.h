...
// TODO: LEARN: the key point is to learn how to use std::enable_if<>
...
#define DEFINE_IMPLICIT_CTOR(type, name)      \
  ...
  // DEFINE_IMPLICIT_CTOR(uint8_t, Byte) and so on
  // ->
  // Scalar(uint8_t vv) : Scalar(uint8_t, true) and so on
...
#undef DEFINE_IMPLICIT_CTOR
  ...
  // problem.
  // TODO: LEARN: https://www.jianshu.com/p/a961c35910d2
  // only valid when T is bool, then the template will be:
  // template<typename T, typename void*=nullptr>
  template <
      typename T,
...
#define DEFINE_ACCESSOR(type, name)                       \
  ...
  /*
  DEFINE_ACCESSOR(uint8, Byte) and so on
  -------------------------------
  uint8 to_Byte() const{
    if (Tag::HAS_d == tag){
      return checked_convert<uint8, double>(v.d, "uint8");
    } ...
    ...
  }
  */
